...
describe("transformForFeatures — §6.2's guard, at the call site", () => {
  ...
  test("lib/mapTransform.ts is not modified by this plan", () => {
    ...
    // `mapTransform.ts:39-47` explains why, so the way this plan could still go
    // wrong is by quietly moving the guard down into the maths — where a second
    // caller would inherit a fallback instead of a visible bug.
    ...
    // Still bare: handed the bounds of nothing, it still answers NaN.
