...
- **~~Keep every country mounted every frame.~~ SUPERSEDED BY G9 AND BY WHAT SHIPPED — do not act on this bullet.**
  It was written from a performance measurement that still stands: mounting all 251 `<g>` nodes versus
  pruning to the front face costs **p50 8.7 ms vs 7.5 ms, p95 13.1 ms vs 11.6 ms**, so 1.2 ms would have
  bought away 430 DOM mount/unmount events per spin. What it got wrong is the conclusion. G9 below settles
  it: `opacity: 0` removes an element from neither the accessibility tree nor the focus order, so keeping
  back-face countries mounted would leave up to 151 phantom `role="button"` nodes a screen reader announces
  as real controls, with no visible focus indicator — WCAG 2.2 AA **2.4.7** and **2.4.11**. `aria-hidden` is
  not an escape hatch either; on a focusable element it is its own violation.
  **What shipped prunes**, and `GlobeLevel.tsx` gives a far-side country no node at all. The stable full list
  lives in `entries` instead, which is what keeps the A–Z picker, the roving tabindex and the selected-country
  card whole as the globe turns. Roughly a dozen tests in `GlobeLevel.test.tsx` assert that far-side countries
  have no node; re-mounting them to honour this bullet would fail those tests and reintroduce three focus defects.
...
| `components/map/worldLevelShared.tsx` | Selection, keyboard, picker, card, error and skeleton — shared by both renderers |
...
- Produces: `WORLD_FIXTURE` gains `NZ` and `PE`; the fixture now spans both hemispheres. Reused by `GlobeLevel.test.tsx` in Task 11.
...
"
```
Expected: `front: FR,MT,PE` and `back : JP,SG,NZ`. If `back` is empty, the rings are still reversed and Task 11's back-face tests would be hollow.
...
## Task 10: Extract the world level's shared parts

`WorldMap.tsx` is 484 lines, and roughly 300 of them are not about Mercator at all: the roving-tabindex machinery, the per-country interaction props, the stroke and opacity rules, the A–Z picker, the selected-country card, the error state and the loading skeleton. The globe needs every one of them, unchanged.

Extracting them now means `GlobeLevel` is written as a projection plus a drag layer, rather than as a second copy of a component that would then have to be kept in step with the first every time the picker's accessibility, tinting or copy changes.

**This is a pure extraction.** All 14 tests in `WorldMap.test.tsx` must stay green **with no assertion edited** — the same oracle discipline as Tasks 7 and 8. If a test has to change, the extraction changed behaviour and is wrong.

**Files:**
- Create: `components/map/worldLevelShared.tsx`
- Modify: `components/map/WorldMap.tsx`

**Interfaces:**
- Consumes: `AccentTheme`, `accentColor` from `@/lib/accent`; `getCountry` from `@/lib/countries`; `usePrefs` from `@/components/shell/PrefsProvider`; `CountryHero` from `@/components/shell/CountryHero`.
- Produces, all from `components/map/worldLevelShared.tsx`:
  - `interface Entry { code: string; name: string }`
  - `countryLabel(code: string, topologyName: string): string`
  - `useCountrySelection(opts): CountrySelection` — see Step 3 for the exact shapes
  - `CountryPicker({ id, entries, selectedCode, onSelectCountry })` — the A–Z `<select>`
  - `SelectedCountryCard({ entry, theme })` — the hero card, or `null` with no entry
  - `WorldLevelError({ onRetry })`
  - `WorldLevelSkeleton()`

- [ ] **Step 1: Confirm the oracle is green before touching anything**

Run: `npx vitest run components/map/WorldMap.test.tsx`
Expected: `14 passed`. Write the number down — it must be identical at the end of this task.

- [ ] **Step 2: Read the code being moved**

Run: `sed -n '113,117p;228,325p;327,364p;425,481p' components/map/WorldMap.tsx`

Everything in this task is a move of that code, verbatim where possible. Do not improve it in passing: a behaviour change hidden inside an extraction is exactly what the green suite cannot catch, because it is checking the behaviour that was there before.
...
**Note for the implementer:** this module has no test file of its own, deliberately. Every behaviour in it is already asserted by the 14 tests in `WorldMap.test.tsx` — which is what makes this extraction verifiable at all — and `GlobeLevel.test.tsx` in Task 11 exercises the second consumer. A new test file here would restate assertions that already exist against a real renderer, which is worse coverage, not more.

- [ ] **Step 4: Rewrite `WorldMap.tsx` to consume it**

Delete from `WorldMap.tsx`: `countryLabel`, the `Entry` interface, `FILL_BASE`/`FILL_HOVER`/`FILL_SELECTED`, `nodeRefs`, the `hoverCode`/`focusedCode`/`activeCode` state, `fillFor`, `tabStop`, `focusEntry`, `stepFor`, `handleKeyDown`, `interactionProps`, `strokeFor`, `opacityFor`, the `loadError` JSX block, the `!view` JSX block, the A–Z `<select>` JSX block, and the `CountryHero` JSX block. Import their replacements:
...
Then wire the hook in, after `view` and `mounted` are computed:
...
and replace the four JSX blocks with:
...
**Hooks-order note:** `useCountrySelection` must be called before the two early returns, not after — React requires an unconditional hook call on every render, and `loadError` / `!view` both short-circuit. The existing early returns already sit below every `useState` and `useMemo` in the file; keep the new hook call above them too.

`WorldMap` keeps its own `containerRef`, `pickerId`, `world`/`loadError`/`retryKey` state, the fetch effect, `UNFITTED`, `POINT_R`, `POINT_HIT_R`, `CountryProps`, `Shape`, `PointMark`, the two memos, and the `<svg>` itself. Those are the flat map.

- [ ] **Step 5: Run the oracle**

Run: `npx vitest run components/map/WorldMap.test.tsx`
Expected: `14 passed` — **the same 14, with no assertion edited.**

The likely failure and what it means: `tints countries from the accent ramp` failing on a null fill means `fillFor` is being read before `usePrefs` resolves — check that the hook is called unconditionally and above the early returns.

- [ ] **Step 6: Confirm the extraction actually shrank the renderer**

Run: `wc -l components/map/WorldMap.tsx components/map/worldLevelShared.tsx`
Expected: `WorldMap.tsx` well under 300 lines, down from 484. If it is still near 484, code was copied rather than moved.
...
```bash
git add components/map/worldLevelShared.tsx components/map/WorldMap.tsx
git commit -m "refactor: extract the world level's shared selection, picker and states"
```

---

## Task 11: The globe renderer
...
Create `components/map/GlobeLevel.tsx`. It is `WorldMap`'s structure with a different projection: the same fetch-parse-memo-render shape, composing the same `worldLevelShared` pieces Task 10 extracted. Six things differ, and nothing else should:
...
5. Focus drives rotation, through the hook's `onFocusOffscreen` callback. `useCountrySelection` already refuses to pretend a missing node took focus; the globe is the renderer that can do something about it:
...
```ts
  const { interactionProps, strokeFor, opacityFor, fillFor, refocus } = useCountrySelection({
    ...
    onFocusOffscreen: (code) => {
      ...
      if (at) spinTo(rotationFor(at[0], at[1]));
    },
  });
  ...
  useEffect(() => {
    ...
    // recreated every render; depending on it would re-focus on every frame
    // of a drag and fight the user for the caret.
  }, [rotation]);
```

`topo` gains a `lonLat: Map<string, [number, number]>`, built from `geoCentroid` of each feature and from each point's own `lon`/`lat`. It belongs in the rotation-independent memo: centroids do not move when the globe turns.
...
## Task 12: Wire the globe into the picker
