...
- **§2.1's "passed down as parameters"** for `enrich-cities` would have changed signatures (its 21 header constants are read inside function bodies); each moved to the one module that reads it, none exported. **§2.1's per-script test split** needed two shared non-test modules for country-facts (`fixtures.ts`, `runHarness.ts`): the kept `run()` test was 960 lines otherwise.
- **§2.2's `MUST_STAY_CHEAP` count is 22** as written here (the plan's self-review misquoted it as 21). Task 12 produced no `report.test.ts` — the original test had no `buildReport` describe; the C7 contract covers it — so the suite is 164 files, not 165. `CountryLevel.tsx` landed at 711 lines against the estimated 745.
